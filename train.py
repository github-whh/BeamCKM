...
parser.add_argument('--dataset', type=str, default='BeamCKM', help='Dataset name')
...
parser.add_argument('--deterministic', type=int, default=1, help='Whether to use deterministic training (1) or not (0)')
parser.add_argument('--base_lr', type=float, default=0.0001, help='Base learning rate for segmentation network')
parser.add_argument('--img_size', type=int, default=256, help='Input patch size for network')
...
parser.add_argument('--vit_patches_size', type=int, default=16, help='ViT patch size (default: 16)')
...
if __name__ == "__main__":
    ...
    snapshot_path = snapshot_path + '_fp16' if args.fp16 else snapshot_path
    ...
    net = ViT_seg(config_vit, img_size=args.img_size, in_channels=3).float()
    
    # Load pretrained model
    net.load_from(weights=np.load("./pretrained/R50+ViT-B_16.npz"))
    trainer = {'CKM': trainer}
    trainer_class = trainer['CKM']
    
    # Train
    trainer_instance = trainer_class(args, net, snapshot_path, accelerator=accelerator)
